...
The standard fix is to teach every application to save and reload its own state. That works, but it's also a per-application tax, paid forever.
...
Removed in favor of what? KEP-5823's real `RestorePod` API. We're living in the window where the by-hand trick works, so let's flip it on:
...
1.6 MiB of that is raw memory pages, and the counter variable is literally in there somewhere.
